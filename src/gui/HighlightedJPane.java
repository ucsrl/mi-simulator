...
/**
 * Diese Klasse stellt den SourceCode eines MI-Programms dar. Dabei
 * wird dieser entsprechend eingefärbt.
...
public class HighlightedJPane extends JTextPane implements KeyListener {
	...
	while (true) {
	    ...
	    highl(ret);
	    	 ...
	    	 setCharacterAttributes(getStyledDocument(), 0,
	    			    this.getText().length(), norm);
    	}
    }
     ...
     * 
     * @param beg
     *            Beginn des Bereiches
     * @param end
     *            End des Bereiches
     * @param diff_opcode
     *            true, wenn der Text rot hervorgehoben werden soll
     */
    public void highlight(int beg, int end, boolean diff_opcode) {
	setCharacterAttributes(getStyledDocument(), highl_beg_old,
		highl_end_old, Enviroment.shl?keyWord:norm);
	highl_beg_old = beg;
	highl_end_old = end;
	...
	if (Enviroment.getNextCommand() != null) {
	    Command next = Enviroment.getCommandperAddress(Enviroment
		    .getNextCommand().getAdress());
	    if (next != null) {
		if (NumberConversion.myByteEqual(next.getOpCode(), Enviroment
			.getNextCommand().getOpCode())
			&& !(next instanceof DD)) {

...
public class RunProgram extends Thread {
     ...
     *            Restart-Button
     */
    public RunProgram(JButton btnRun, JButton btnStop, JButton btnStep,
	    JButton btnRestart) {
	button_run = btnRun;
	button_stop = btnStop;
	button_step = btnStep;
	...
	while (next != null  && !stop_program && !breakPoint) {
	    ...
	    if (next != null) {
		Command next_precode = Enviroment.getCommandperAddress(next
			.getAdress());
		if (next_precode != null
			&& (NumberConversion.myByteEqual(next.getOpCode(),
				next_precode.getOpCode()))
			) {

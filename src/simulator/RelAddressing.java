...
public class RelAddressing implements Operand, AdressGetter {
     ...
     *            Länge des Registerzugriffs
     * @param address
     *            Adresse des Befehls the adress
     */
	...
	if (nr1 == CONSTANTS.PC_REGISTER) {
	    Register index = nr2 != -1 ? Enviroment.REGISTERS.getRegister(nr2)
		    : null;
	    return index != null ? new AbsAddress(address
		    + offset
		    + length
		    * NumberConversion.myBytetoIntWithoutSign(index
			    .getContent(4)), length, 0).getContent()
		    : new AbsAddress(address + offset, length, 0).getContent();
	}
	Register index = nr2 != -1 ? Enviroment.REGISTERS.getRegister(nr2)
		: null;
	Register register = Enviroment.REGISTERS.getRegister(nr1);
	return index != null ? new AbsAddress(
	...
	return index == null ? new MyByte[] { new MyByte(14 * 16 + nr1),
		...
		new MyByte(4 * 16 + nr2), new MyByte(14 * 16 + nr1),
		NumberConversion.intToByte(offset, 4)[0],
		NumberConversion.intToByte(offset, 4)[1],
		NumberConversion.intToByte(offset, 4)[2],
	...
	if (nr1 == CONSTANTS.PC_REGISTER) {
	    Register index = nr2 != -1 ? Enviroment.REGISTERS.getRegister(nr2)
		    : null;
	    AbsAddress ort = index != null ? new AbsAddress(address
		    + offset
		    + length
		    * NumberConversion.myBytetoIntWithoutSign(index
			    .getContent(4)), length, 0) : new AbsAddress(address
		    + offset, 4, 0);
	    ...
	    Register index = nr2 != -1 ? Enviroment.REGISTERS.getRegister(nr2)
		    : null;
	    Register register = Enviroment.REGISTERS.getRegister(nr1);
	    AbsAddress ort = index != null ? new AbsAddress(
		    NumberConversion.myBytetoIntWithoutSign(register
			    .getContent(4))
			    + offset
			    + length
			    * NumberConversion.myBytetoIntWithoutSign(index
				    .getContent(4)), length, 0)
		    : new AbsAddress(
			    NumberConversion.myBytetoIntWithoutSign(register
				    .getContent(4)) + offset, length, 0);
	    ort.setContent(content, length);

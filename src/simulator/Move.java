...
public class Move extends Command {
     ...
     *            true, falls es ein Gleitpunktzahlbefehl ist
     */
    public Move(int line, int adress, int length, Operand op1, Operand op2,
	...
	if (floating) {
	    switch (length) {
	    case 4:
		zero = Float.intBitsToFloat(NumberConversion
			.myBytetoIntWithoutSign(merke)) == 0;
		negative = Float.intBitsToFloat(NumberConversion
			.myBytetoIntWithoutSign(merke)) < 0;
		break;
	    case 8:
		zero = Double.longBitsToDouble(NumberConversion
			.myBytetoLongWithoutSign(merke)) == 0;
		negative = Double.longBitsToDouble(NumberConversion
			.myBytetoLongWithoutSign(merke)) < 0;
		break;
	    }
	} else {

...
public class Div extends Command {
     ...
     *            true, falls es ein Gleitpunktzahlbefehl ist
     */
    public Div(int line, int adress, int length, Operand op1, Operand op2,
    ...
    public Div(int line, int adress, int length, Operand op1, Operand op2,
	    Operand op3, int beg, int end, boolean floating) {
	...
	case 2:
	    ...
	    break;
	case 4:
	    opcode = three ? (floating ? new MyByte("E5") : new MyByte("E4"))
	...
	if (floating) {
		...
		Float ope1 = Float.intBitsToFloat(NumberConversion
			.myBytetoIntWithoutSign(op1.getContent()));
		Float ope2 = Float
			.intBitsToFloat(NumberConversion
				.myBytetoIntWithoutSign(((three == false) && (op2 instanceof CellarAddressing)) ? ((CellarAddressing) op2)
					.getContentWithoutOffset() : op2
					.getContent()));
		...
		Enviroment.flags.setOverflow(Float.isInfinite(erg_1)
			|| erg_1 == 0 && ope1 != 0 && ope2 != 0);

		break;
	    case 8:

		Double ope11 = Double.longBitsToDouble(NumberConversion
			.myBytetoLongWithoutSign(op1.getContent()));
		Double ope22 = Double
			.longBitsToDouble(NumberConversion
				.myBytetoLongWithoutSign(((three == false) && (op2 instanceof CellarAddressing)) ? ((CellarAddressing) op2)
					.getContentWithoutOffset() : op2
					.getContent()));
		Double erg_11 = ope22 / ope11;
		erg = NumberConversion.intToByte(
			Double.doubleToLongBits(erg_11), length);
	    ...
	    Enviroment.flags.setZero(NumberConversion
		    .myBytetoIntWithoutSign(erg) == 0);

	    Enviroment.flags.setNegative(NumberConversion
		    .myBytetoIntWithoutSign(erg) < 0);

	    Enviroment.flags.setOverflow(!NumberConversion.valid_number(erg1,
		    length));
	...
	return "DIV " + bhwfd + " " + op1.toString() + ", " + op2.toString()
		+ (three ? ", " + op3.toString() : "");
    }

...
public class MoveN extends Command {
     ...
     *            true, falls es ein Gleitpunktzahlbefehl ist
     */
    public MoveN(int line, int adress, int length, Operand op1, Operand op2,
	...
	if (floating) {
	    switch (length) {
	    case 4:
		ret = NumberConversion.intToByte(Float.floatToIntBits(Float
			.intBitsToFloat(NumberConversion
				.myBytetoIntWithSign(op1.getContent()))
			* -1), length);
		break;
	    case 8:
		ret = NumberConversion.intToByte(Double.doubleToLongBits(Double
			.longBitsToDouble(NumberConversion.myBytetoLongWithoutSign(op1
				.getContent()))
			* -1), length);
		break;
	    }
	} else {
	    ret = NumberConversion
		    .intToByte(
			    NumberConversion.myBytetoIntWithSign(op1
				    .getContent()) * -1, length);
	}

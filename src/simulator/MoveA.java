...
public class MoveA extends Command {
     ...
     */
    public MoveA(int line, int adress, Operand op1, Operand op2, int beg,
	    int end) {
	...
	op2.setContent(
		NumberConversion.intToByte(
			(op1 instanceof AbsAddress && ((AbsAddress) op1)
				.hasLabel()) ? ((AbsAddress) op1).getAdress()
	...
	if (op2 instanceof AbsAddress && ((AbsAddress) op2).hasLabel()) {
	    ((AbsAddress) op2).setOrt(((AbsAddress) op1).getOrt()
		    + op1.getOpCode().length);
	}

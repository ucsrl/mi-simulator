...
import simulator.AndNot;
import simulator.Call;
import simulator.CellarAddressing;
import simulator.Clear;
import simulator.Cmp;
import simulator.Command;
import simulator.Conv;
import simulator.DD;
import simulator.Div;
import simulator.Ext;
import simulator.Exts;
import simulator.Findc;
import simulator.Finds;
import simulator.Halt;
import simulator.ImmediateOperand;
import simulator.IndAddressing;
import simulator.Ins;
import simulator.Jbcci;
import simulator.Jbssi;
import simulator.Jump;
import simulator.Move;
import simulator.MoveA;
import simulator.MoveC;
import simulator.MoveN;
import simulator.Mult;
import simulator.Operand;
import simulator.Or;
import simulator.Popr;
import simulator.Pushr;
import simulator.RegisterAddressing;
import simulator.RelAddressing;
import simulator.Res;
import simulator.Ret;
import simulator.Rot;
import simulator.Sh;
import simulator.Sub;
import simulator.Xor;
...
public class Parser {
	...
	case 2:
	    befehl = new Add(zeile, adress, length, operands.get(0),
		    operands.get(1), beg, end, floating);
	    break;
	case 3:
	    befehl = new Add(zeile, adress, length, operands.get(0),
		    operands.get(1), operands.get(2), beg, end, floating);
	    break;
	default:
	...
	switch (operands.size()) {
	case 2:
	    befehl = new AndNot(zeile, adress, length, operands.get(0),
		    operands.get(1), beg, end);
	    break;
	case 3:
	    befehl = new AndNot(zeile, adress, length, operands.get(0),
		    operands.get(1), operands.get(2), beg, end);
	    break;
	default:
	    error.append(zeile, CONSTANTS.ERROR_PARSER_NUMBER_OF_OPERANDS);
	}
	...
	switch (operands.size()) {
	case 1:
	    befehl = new Clear(zeile, adress, length, operands.get(0), beg,
		    end, floating);
	    break;
	default:
	    error.append(zeile, CONSTANTS.ERROR_PARSER_NUMBER_OF_OPERANDS);
	...
	switch (operands.size()) {
	case 2:
	    befehl = new Cmp(zeile, adress, length, operands.get(0),
		    operands.get(1), beg, end, floating);
	    break;
	default:
	    error.append(zeile, CONSTANTS.ERROR_PARSER_NUMBER_OF_OPERANDS);
	...
	switch (operands.size()) {
	case 2:
	    befehl = new Div(zeile, adress, length, operands.get(0),
		    operands.get(1), beg, end, floating);
	    break;
	case 3:
	    befehl = new Div(zeile, adress, length, operands.get(0),
	...
	switch (operands.size()) {
	case 1:
	    befehl = new Jump(zeile, adress, operands.get(0), typ, beg,
		    end);
	    break;
	...
	switch (operands.size()) {
	case 2:
	    befehl = new Move(zeile, adress, length, operands.get(0),
		    operands.get(1), beg, end, floating);
	    break;
	default:
	    error.append(zeile, CONSTANTS.ERROR_PARSER_NUMBER_OF_OPERANDS);
	...
	switch (operands.size()) {
	case 2:
	    if (operands.get(0) instanceof RelAddressing
		    || operands.get(0) instanceof IndAddressing
		    || (operands.get(0) instanceof AbsAddress && ((AbsAddress) operands
			    .get(0)).hasLabel())) {
		befehl = new MoveA(zeile, adress, operands.get(0),
			operands.get(1), beg, end);
	    } else {
		error.append(zeile,
	...
	switch (operands.size()) {
	case 2:
	    befehl = new MoveC(zeile, adress, length, operands.get(0),
		    operands.get(1), beg, end);
	    break;
	...
	switch (operands.size()) {
	case 2:
	    befehl = new MoveN(zeile, adress, length, operands.get(0),
		    operands.get(1), beg, end, floating);
	    break;
	default:
	    error.append(zeile, CONSTANTS.ERROR_PARSER_NUMBER_OF_OPERANDS);
	...
	switch (operands.size()) {
	case 2:
	    befehl = new Mult(zeile, adress, length, operands.get(0),
		    operands.get(1), beg, end, floating);
	    break;
	case 3:
	    befehl = new Mult(zeile, adress, length, operands.get(0),
    ...
    /**
     * Diese Methode parsed die Operanden solange, bis ein EndOfCommand-Zeichen erkannt wird
     * 
     * @param length
     *            Typangabe des Befehls
     * @param floating
     *            Gleitpunktzahlenbefehl
	...
	while (true) {
		...
		while (ret.getNr() == Scanner.TOKEN_NUMBER) {
		    off += Integer.parseInt(ret.getText().substring(0, 1)
			    .equals("+") ? ret.getText().substring(1,
			    ret.getText().length()) : ret.getText());
		    ret = scanner.getNextSymbol();
		}
		...
		list.add(new AbsAddress(Integer.parseInt(ret.getText()
			.substring(2, ret.getText().length() - 1), 2), 4,
			adress));
		...
		list.add(new AbsAddress(Integer.parseInt(ret.getText()
			.substring(2, ret.getText().length() - 1), 16), 4,
			adress));
		ret = scanner.getNextSymbol();
		break;
		...
		case Scanner.TOKEN_REGISTER:
		    ...
		    if (ret.equal(new int[] { Scanner.TOKEN_COMMA,
			    Scanner.TOKEN_NEWLINE, Scanner.TOKEN_APOSTROPHE,
			    Scanner.TOKEN_EOF })) {
			list.add(new RelAddressing(0, reg, length, 0));
		    } else {
			if (ret.getNr() == Scanner.TOKEN_PLUS) {
			    list.add(new CellarAddressing(reg, length, length,
				    true));
			    ret = scanner.getNextSymbol();
			} else {
				...
				if (ret.getNr() == Scanner.TOKEN_REGISTER) {
				    index = Integer.parseInt(ret.getText()
					    .substring(1,
						    ret.getText().length()));
				    ret = scanner.getNextSymbol();
				    if (ret.getNr() == Scanner.TOKEN_SLASH) {
					list.add(new RelAddressing(0, reg,
						index, length, 0));
					ret = scanner.getNextSymbol();
				    }
				}
		...
		case Scanner.TOKEN_EXCLAMATIONPOINT:
			...
			if (ret.equal(new int[] { Scanner.TOKEN_COMMA,
				...
				if (ret.getNr() == Scanner.TOKEN_REGISTER) {
				    ...
				    ret = scanner.getNextSymbol();
				    if (ret.getNr() == Scanner.TOKEN_SLASH) {
					list.add(new IndAddressing(0, reg,
						index, length));
					ret = scanner.getNextSymbol();
				...
				if (ret.getNr() == Scanner.TOKEN_REGISTER) {
				    reg = Integer.parseInt(ret.getText()
					    .substring(1,
						    ret.getText().length()));
				    ret = scanner.getNextSymbol();
				    if (ret.getNr() == Scanner.TOKEN_BRACKETCLOSE) {
					ret = scanner.getNextSymbol();
					if (ret.equal(new int[] {
						...
						Scanner.TOKEN_EOF })) {
					    list.add(new IndAddressing(offset,
						    reg, length, 0));
					} else {
					    if (ret.getNr() == Scanner.TOKEN_SLASH) {
						ret = scanner.getNextSymbol();
		...
		if (ret.equal(new int[] { Scanner.TOKEN_COMMA,
			...
			if (ret.getNr() == Scanner.TOKEN_EXCLAMATIONPOINT) {
			    ret = scanner.getNextSymbol();
			    if (ret.getNr() == Scanner.TOKEN_REGISTER) {
				reg = Integer.parseInt(ret.getText().substring(
					1, ret.getText().length()));
				ret = scanner.getNextSymbol();
				if (ret.equal(new int[] { Scanner.TOKEN_COMMA,
					Scanner.TOKEN_NEWLINE,
					Scanner.TOKEN_APOSTROPHE,
					Scanner.TOKEN_EOF })) {
				    list.add(new RelAddressing(offset, reg,
					    length, 0));
				} else {
				    if (ret.getNr() == Scanner.TOKEN_SLASH) {
					ret = scanner.getNextSymbol();
					if (ret.getNr() == Scanner.TOKEN_REGISTER) {
					    index = Integer.parseInt(ret
						    .getText().substring(
							    1,
							    ret.getText()
								    .length()));
		...
		case Scanner.TOKEN_BIN:
		    ...
		    zahl = Long.valueOf(
			    ret.getText().substring(2,
				    ret.getText().length() - 1), 2);
		...
		case Scanner.TOKEN_NUMBER:
			...
			if (!NumberConversion.valid_number(zahl, length))
			    error.append(zeile,
				    CONSTANTS.ERROR_PARSER_NO_VALIDNUMBER);
			list.add(new ImmediateOperand(NumberConversion
				.intToByte(Integer.valueOf(ret.getText()),
					length), length));
			ret = scanner.getNextSymbol();
		    } else {
			switch (length) {
			case 4:
				...
				list.add(new ImmediateOperand(NumberConversion
					.intToByte(Float.floatToIntBits(a),
						length), length));
				ret = scanner.getNextSymbol();
			...
			case 8:
				...
				list.add(new ImmediateOperand(NumberConversion
					.intToByte(Double.doubleToLongBits(a),
						length), length));
				ret = scanner.getNextSymbol();
			    } catch (Exception e) {
				error.append(zeile,
		...
		case Scanner.TOKEN_FLOAT:
		    if (!floating) {
			error.append(zeile, CONSTANTS.ERROR_PARSER_NO_FLOAT);
			ret = scanner.getNextSymbol();
		    } else {
			switch (length) {
			case 4:
		...
		if (ret.getNr() == Scanner.TOKEN_EXCLAMATIONPOINT) {
		    ret = scanner.getNextSymbol();
		    if (ret.getNr() == Scanner.TOKEN_REGISTER) {
			list.add(new CellarAddressing(Integer
				.parseInt(ret.getText().substring(1,
					ret.getText().length())), length,
				-length, false));
			ret = scanner.getNextSymbol();
		    } else {
			error.append(zeile, CONSTANTS.ERROR_PARSER_NO_REGISTER);
		    }
		} else {
		    error.append(zeile, CONSTANTS.ERROR_PARSER_FALSE_SYMBOL);
		}
	...
	while (true) {
		...
		zahl = Integer.parseInt(
			ret.getText().substring(2, ret.getText().length() - 1),
			2);
		newlength = length == 0 ? getLengthForOperand(zahl) : length;
		list.add(new ImmediateOperand(NumberConversion.intToByte(zahl,
			newlength), newlength));
		ret = scanner.getNextSymbol();
		break;
		...
		case 4:
			...
			Float a = Float.parseFloat(ret.getText());
			list.add(new ImmediateOperand(NumberConversion
				.intToByte(Float.floatToIntBits(a), length),
				length));
			ret = scanner.getNextSymbol();

		    } catch (Exception e) {
			error.append(zeile, CONSTANTS.ERROR_PARSER_PARSE_FLOAT);
		...
		case 8:
			...
			Double a = Double.parseDouble(ret.getText());
			list.add(new ImmediateOperand(NumberConversion
				.intToByte(Double.doubleToLongBits(a), length),
				length));
			ret = scanner.getNextSymbol();
		    } catch (Exception e) {
			error.append(zeile, CONSTANTS.ERROR_PARSER_PARSE_DOUBLE);
		    }
		    break;
		default:
		    error.append(zeile,
		...
		if (!floating) {
		    ...
		    newlength = length == 0 ? getLengthForOperand(zahl)
			    : length;
		    list.add(new ImmediateOperand(NumberConversion.intToByte(
			    zahl, newlength), newlength));
		    ret = scanner.getNextSymbol();
		} else {
		    switch (length) {
		    case 4:
	...
	switch (operands.size()) {
	case 2:
	    befehl = new Or(zeile, adress, length, operands.get(0),
		    operands.get(1), beg, end);
	    break;
	case 3:
	    befehl = new Or(zeile, adress, length, operands.get(0),
	...
	switch (operands.size()) {
	case 2:
	    befehl = new Conv(zeile, adress, operands.get(0), operands.get(1),
		    beg, end);
	    break;
	default:
	    error.append(zeile, CONSTANTS.ERROR_PARSER_NUMBER_OF_OPERANDS);
    ...
    public void start() {
	ret = scanner.getNextSymbol();
	while (ret.getNr() != Scanner.TOKEN_EOF
		&& ret.getNr() != Scanner.TOKEN_FEHLER) {
	...
	switch (operands.size()) {
	case 2:
	    befehl = new Sub(zeile, adress, length, operands.get(0),
		    operands.get(1), beg, end, floating);
	    break;
	case 3:
	    befehl = new Sub(zeile, adress, length, operands.get(0),
	...
	switch (operands.size()) {
	case 2:
	    befehl = new Xor(zeile, adress, length, operands.get(0),
		    operands.get(1), beg, end);
	    break;
	case 3:
	    befehl = new Xor(zeile, adress, length, operands.get(0),
	...
	while (!ret.equal(new int[]{Scanner.TOKEN_APOSTROPHE,Scanner.TOKEN_NEWLINE, Scanner.TOKEN_EOF})) {
	    ret=scanner.getNextSymbol();
	}
    }

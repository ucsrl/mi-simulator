...
public class Interpreter {
	...
	if (opcode < 64) {
	    return new ImmediateOperand(NumberConversion.intToByte(opcode,
		    length), length);
	}
	...
	if (opcode == 143) {
	    ...
	    return new ImmediateOperand(Enviroment.MEMORY.getContent(
		    Enviroment.getPC() - length, length), length);
	...
	if (opcode == 159) {
	    ...
	    return new AbsAddress(
		    NumberConversion.myBytetoIntWithSign(Enviroment.MEMORY
			    .getContent(Enviroment.getPC() - 4, 4)), length,
		    Enviroment.getPC() - 4);
	}
	...
	if (opcode >= 64 && opcode <= 79) {
		...
		Enviroment.addToPC(1);
		return new RelAddressing(
			NumberConversion.myBytetoIntWithSign(Enviroment.MEMORY
				.getContent(Enviroment.getPC() - 1, 1)),
			opcode - 160, index, length, Enviroment.getPC() - 1);
	    }
		...
		return new RelAddressing(
			NumberConversion.myBytetoIntWithSign(Enviroment.MEMORY
				.getContent(Enviroment.getPC() - 2, 2)),
			opcode - 192, index, length, Enviroment.getPC() - 2);
	    }
		...
		return new RelAddressing(
			NumberConversion.myBytetoIntWithSign(Enviroment.MEMORY
				.getContent(Enviroment.getPC() - 4, 4)),
			opcode - 224, index, length, Enviroment.getPC() - 4);
	    }

	    // Indirekte Adressierung mit Offset im byte darstellbar
	    if (opcode >= 176 && opcode <= 191) {
		Enviroment.addToPC(1);
		return new IndAddressing(
			NumberConversion.myBytetoIntWithSign(Enviroment.MEMORY
				.getContent(Enviroment.getPC() - 1, 1)),
			opcode - 176, index, length, Enviroment.getPC() - 1);
	    }
		...
		return new IndAddressing(
			NumberConversion.myBytetoIntWithSign(Enviroment.MEMORY
				.getContent(Enviroment.getPC() - 2, 2)),
			opcode - 208, index, length, Enviroment.getPC() - 2);
	    }
		...
		return new IndAddressing(
			NumberConversion.myBytetoIntWithSign(Enviroment.MEMORY
				.getContent(Enviroment.getPC() - 4, 4)),
			opcode - 240, index, length, Enviroment.getPC() - 4);
	    }
	    return null;
	}

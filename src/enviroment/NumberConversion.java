...
public class NumberConversion {
	...
	for (int i = op1.length - 1; i >= 0; i--) {
	    ret[i] = carry ? new MyByte(op1[i].getContent()
		    + op2[i].getContent() + 1) : new MyByte(op1[i].getContent()
		    + op2[i].getContent());
	...
	if (neg) {
		...
		ret[i] = new MyByte(Integer.parseInt(
			neu.substring(i * 8, i * 8 + 8), 2));
	    }
	} else {
	    for (int i = 0; i < length; i++) {
	...
	if (in[0].getContent() > 127) {
	    neg = true;
	    for (int i = in.length - 1; i >= 0; i--) {
		ret[i] = carry ? new MyByte(in[i].negate().getContent() + 1)
			: new MyByte(in[i].negate().getContent());
		carry = ret[i].getCarry();

	    }
	}
	return neg ? (-1 * myBytetoIntWithoutSign(ret))
		: myBytetoIntWithoutSign(ret);
    }
     ...
     *            zu pruefende Zahl
     * @param length
     *            Zahlenraum (0: byte, 2; short, 4: integer)
     * @return gueltig
     */
